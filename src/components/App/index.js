...
Object.keys(globals).forEach(rule => {
  css.global(rule, globals[rule]);
});
      ...
      <IntlProvider locale={this.state.locale} messages={getMessages(this.state.locale)}>
        <div className={css(styles.page)}>
          ...
          <div className={css(styles.content)}>
            <section className={css(styles.primary)}>
              {children}
            </section>
            <section className={css(styles.secondary)}>
              <Sidebar sidebar={sidebar} />
            </section>
          </div>
          <footer className={css(styles.footerCredits)}>
            © Scott Taylor ...&nbsp; Brooklyn, NY ...{' '}
            <a href="https://twitter.com/wonderboymusic">@wonderboymusic</a>&nbsp; ... Powered by
            GraphQL / React / Relay / WordPress / nginx / Redis / SCSS
          </footer>
        </div>
      </IntlProvider>
    );
